...
# By default, `kaspazd` inside the container runs as the `nobody:nogroup` user. All files
...
if [[ "$nocheck" -eq 0 && "$(stat -c '%u:%g' /data)" != "65534:65534" ]]; then
  echo "Folder mounted at /data is not owned by nobody:nogroup, please change its permissions on the host with 'sudo chown -R 65534:65534 path/to/kaspaz-data'."
  exit 1
fi

/app/fetch-params.sh
exec /app/kaspazd -datadir=/data/.kaspaz "$@"
