...
Section "Install Komodo-Qt (GUI)" Section1
  SetOutPath $INSTDIR
  File "content\kaspaz-qt-win.exe"
  CreateShortCut "$DESKTOP\Komodo-Qt.lnk" "$INSTDIR\kaspaz-qt-win.exe"
SectionEnd
...
Section "Create kaspaz.conf" Section2
  SetOverwrite on
  CreateDirectory "$APPDATA\Komodo"  
  SetOutPath $APPDATA\Komodo
  File "content\kaspaz.conf"
SectionEnd