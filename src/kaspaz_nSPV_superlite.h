...
// kaspaz_nSPVresp is called from async message processing

void kaspaz_nSPVresp(CNode *pfrom,std::vector<uint8_t> response) // received a response
{
...
// kaspaz_nSPV from main polling loop (really this belongs in its own file, but it is so small, it ended up here)

void kaspaz_nSPV(CNode *pto) // polling loop from SendMessages
{
