...
int32_t kaspaz_waituntilelegible(uint32_t blocktime, int32_t stakeHeight, uint32_t delay)
{
...
CBlockTemplate* CreateNewBlock(const CPubKey _pk,const CScript& _scriptPubKeyIn, int32_t gpucount, bool isStake)
{
    ...
    CBlockIndex* pindexPrev = 0;
    {
        ...
        if ( ASSETCHAINS_NOTARY_PAY[0] != 0 )
        {
            ...
            numSN = kaspaz_notaries(notarypubkeys, nHeight, pblock->nTime);
        }
             ...
             mi != mempool.mapTx.end(); ++mi)
        {
            ...
            if (chainName.isKMD() && !kaspaz_validate_interest(tx, nHeight, cmptime))
            {
                LogPrintf("%s: kaspaz_validate_interest failure txid.%s nHeight.%d nTime.%u vs locktime.%u (cmptime.%lu)\n", __func__, tx.GetHash().ToString(), nHeight, (uint32_t)pblock->nTime, (uint32_t)tx.nLockTime, cmptime);
                continue;
            }
            ...
            if (tx.IsCoinImport())
            {
                ...
                if ( numSN != 0 && notarypubkeys[0][0] != 0 && kaspaz_is_notarytx(tx) == 1 )
            ...
            if ( fNotarisation ) 
            {
                ...
                if ( tx.vout.size() == 2 && tx.vout[1].nValue == 0 )
                {
                    ...
                    if ( script[0] == OP_RETURN )
                    {
                        ...
                        int32_t notarizedheight = kaspaz_getnotarizedheight(pblock->nTime, nHeight, script, scriptlen);
        ...
        if ( !chainName.isKMD() && isStake )
        {
            ...
            blocktime = GetTime();
            uint256 merkleroot = kaspaz_calcmerkleroot(pblock, pindexPrev->GetBlockHash(), nHeight, true, scriptPubKeyIn);
            ...
            */
            ...
            siglen = kaspaz_staked(txStaked, pblock->nBits, &blocktime, &txtime, &utxotxid, &utxovout, &utxovalue, utxosig, merkleroot);
            if ( kaspaz_newStakerActive(nHeight, blocktime) != 0 )
                nFees += utxovalue;
            ...
            if ( kaspaz_waituntilelegible(blocktime, stakeHeight, delay) == 0 )
                return(0);
        ...
        uint64_t commission;
        if ( nHeight > 1 && !chainName.isKMD() && (ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 || ASSETCHAINS_SCRIPTPUB.size() > 1) && (ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0)  && (commission= kaspaz_commission((CBlock*)&pblocktemplate->block,(int32_t)nHeight)) != 0 )
        {
            uint8_t *ptr;
            txNew.vout.resize(2);
            ...
            if ( ASSETCHAINS_SCRIPTPUB.size() > 1 )
            {
                ...
                if ( !didinit && nHeight > KOMODO_EARLYTXID_HEIGHT && KOMODO_EARLYTXID != zeroid && kaspaz_appendACscriptpub() )
                {
        ...
        else if ( (uint64_t)(txNew.vout[0].nValue) >= ASSETCHAINS_TIMELOCKGTE)
        {
            ...
            opretScript.AddCheckLockTimeVerify(kaspaz_block_unlocktime(nHeight));
        ...
        else if ( fNotarisationBlock && ASSETCHAINS_NOTARY_PAY[0] != 0 && pblock->vtx[1].vout.size() == 2 && pblock->vtx[1].vout[1].nValue == 0 )
        {
            ...
            if ( script[0] == OP_RETURN )
            {
                uint64_t totalsats = kaspaz_notarypay(txNew, NotarisationNotaries, pblock->nTime, nHeight, script, scriptlen);
                if ( totalsats == 0 )
                {
        ...
        if ( chainName.isKMD() && IS_KOMODO_NOTARY && My_notaryid >= 0 )
        {
            ...
            if ( Mining_height > nDecemberHardforkHeight ) //December 2019 hardfork
                opret = kaspaz_makeopret(pblock, true);
            else
                opret.clear();

            if (kaspaz_notaryvin(txNotary, NOTARY_PUBKEY33, opret, pblock->nTime) > 0)
            {
                CAmount txfees = 5000;
...
void kaspaz_sendmessage(int32_t minpeers,int32_t maxpeers,const char *message,std::vector<uint8_t> payload)
{
    int32_t numsent = 0;
    LOCK(cs_vNodes);
...
void kaspaz_broadcast(CBlock *pblock,int32_t limit)
{
...
#endif
{
    ...
    while ( (ASSETCHAIN_INIT == 0 || KOMODO_INITDONE == 0) )
    {
        ...
        if ( kaspaz_baseid(chainName.symbol().c_str()) < 0 )
            break;
    }
    if ( chainName.isKMD() )
    {
        ...
        kaspaz_chosennotary(&notaryid,newHeight,NOTARY_PUBKEY33,timePast);
    }
...
//                                                                       [&m_cs, &cancelSolver](const uint256& hashNewTip) mutable {
    ...
    try {
        ...
        while (true)
        {
            ...
            if ( (chainName.isKMD() && notaryid >= 0 && Mining_height > nDecemberHardforkHeight ) || (ASSETCHAINS_STAKED != 0 && kaspaz_newStakerActive(Mining_height, pblock->nTime) != 0) ) //December 2019 hardfork
                nExtraNonce = 0;
            ...
            if ( chainName.isKMD() && notaryid >= 0 )
            {
                ...
                if ( (Mining_height >= 235300 && Mining_height < 236000) || (Mining_height % KOMODO_ELECTION_GAP) > 64 || (Mining_height % KOMODO_ELECTION_GAP) == 0 || Mining_height > 1000000 )
                {
                    ...
                    if ( notaryid <= 3 || notaryid == 32 || (notaryid >= 43 && notaryid <= 45) || notaryid == 51 || notaryid == 52 || notaryid == 56 || notaryid == 57 )
                        dispflag = 1;
                    kaspaz_eligiblenotary(pubkeys,mids,blocktimes,&nonzpkeys,Mining_height);
                    if ( nonzpkeys > 0 )
                    {
                        for (i=0; i<33; i++)
                            if( pubkeys[0][i] != 0 )
            ...
            if ( ASSETCHAINS_STAKED > 0 )
            {
                ...
                HASHTarget_POW = kaspaz_PoWtarget(&percPoS,HASHTarget,Mining_height,ASSETCHAINS_STAKED,kaspaz_newStakerActive(Mining_height, pblock->nTime));
                ...
                (std::vector<unsigned char> soln) {
                    ...
                    else
                    {
                        if ( KOMODO_MININGTHREADS == 0 ) // we are staking 
                        {
                            ...
                            if ( ASSETCHAINS_STAKED < 100 && kaspaz_newStakerActive(Mining_height,pblock->nTime) != 0 && h < hashTarget_POW )
                            {
                            ...
                            if ( kaspaz_waituntilelegible(B.nTime, Mining_height, ASSETCHAINS_STAKED_BLOCK_FUTURE_MAX) == 0 )
                                return(false);
                        }
