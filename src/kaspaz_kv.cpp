...
#include "kaspaz_curve25519.h" // for kaspaz_kvsigverify
#include <mutex>
 ...
 */
uint256 kaspaz_kvsig(uint8_t *buf,int32_t len,uint256 _privkey)
{
 ...
 */
int32_t kaspaz_kvnumdays(uint32_t flags)
{
    int32_t numdays;
...
{
    return(kaspaz_kvnumdays(flags) * KOMODO_KVDURATION);
}
...
uint64_t kaspaz_kvfee(uint32_t flags,int32_t opretlen,int32_t keylen)
{
    ...
    if ( (k= keylen) > 32 )
        k = 32;
    numdays = kaspaz_kvnumdays(flags);
 ...
 */
int32_t kaspaz_kvsearch(uint256 *pubkeyp, int32_t current_height, uint32_t *flagsp,
        int32_t *heightp, uint8_t value[IGUANA_MAXSCRIPTSIZE], uint8_t *key, int32_t keylen)
{
    ...
    *flagsp = 0;
    ...
    if ( ptr != nullptr )
    {
        int32_t duration = kaspaz_kvduration(ptr->flags);
 ...
 */
void kaspaz_kvupdate(uint8_t *opretbuf,int32_t opretlen,uint64_t value)
{
    static uint256 zeroes;
    ...
    if ( keylen+13 > opretlen )
    {
        ...
        if ( ++counter < 1 )
            LogPrintf("kaspaz_kvupdate: keylen.%d + 13 > opretlen.%d, this can be ignored\n",keylen,opretlen);
        return;
    }
    uint8_t *valueptr = &key[keylen];
    uint64_t fee = kaspaz_kvfee(flags,opretlen,keylen);
    if ( value >= fee )
    {
                ...
                || opretlen == coresize+2*sizeof(uint256) )
        {
            ...
            uint256 refpubkey;
            int32_t kvheight;
            int32_t refvaluesize = kaspaz_kvsearch(&refpubkey,height,
            ...
            else if ( ptr == 0 )
            {
                ...
                ptr = (kaspaz_kv *)calloc(1,sizeof(*ptr));
