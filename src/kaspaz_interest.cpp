...
#include "kaspaz_utils.h" // dstr()
#include "kaspaz_hardfork.h"

#define KOMODO_INTEREST ((uint64_t)5000000) //((uint64_t)(0.05 * COIN))   // 5%

uint64_t _kaspaz_interestnew(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    int32_t minutes; 
 ...
 */
uint64_t kaspaz_interestnew(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
    if ( txheight < KOMODO_ENDOFERA 
            ...
            && nLockTime < tiptime 
            && nValue >= 10*COIN )
        return _kaspaz_interestnew(txheight,nValue,nLockTime,tiptime);
    return 0;
}
 ...
 */
uint64_t kaspaz_interest(int32_t txheight,uint64_t nValue,uint32_t nLockTime,uint32_t tiptime)
{
            ...
            if ( nValue > 25000LL*COIN )
            {
                ...
                if ( !exception )
                {
                    ...
                    else if ( txheight < 1000000 )
                    {
                        ...
                        uint64_t interestnew = _kaspaz_interestnew(txheight,nValue,nLockTime,tiptime);
                        ...
                        interest = _kaspaz_interestnew(txheight,nValue,nLockTime,tiptime);
                }
                else if ( txheight < 1000000 )
                {
 ...
 * @returns locktime
 */
uint32_t kaspaz_interest_args(uint32_t *txheighttimep,int32_t *txheightp,uint32_t *tiptimep,uint64_t *valuep,
        uint256 hash,int32_t n)
{
    ...
    *valuep = 0;
    ...
    if ( n < tx.vout.size() )
    {
        CBlockIndex *pindex = kaspaz_getblockindex(hashBlock);
        if ( pindex != nullptr )
        {
 ...
 */
uint64_t kaspaz_accrued_interest(int32_t *txheightp,uint32_t *locktimep,uint256 hash,int32_t n,
        int32_t checkheight,uint64_t checkvalue,int32_t tipheight)
{
    ...
    uint64_t value;
    *locktimep = kaspaz_interest_args(&txheighttimep, txheightp, &tiptime, &value, hash, n);
    if ( *locktimep != 0 )
    {
        if ( (checkvalue == 0 || value == checkvalue) && (checkheight == 0 || *txheightp == checkheight) )
            return kaspaz_interest(*txheightp,value,*locktimep,tiptime);
        else 
            LogPrintf("kaspaz_accrued_interest value mismatch %llu vs %llu or height mismatch %d vs %d\n",(long long)value,(long long)checkvalue,*txheightp,checkheight);
    }
    return 0;
}
