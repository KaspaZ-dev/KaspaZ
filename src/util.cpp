...
/****
 * @brief get the OS-specific default kaspazd data directory
...
{    
    ...
    // Unix
    if (chainName.isKMD())
        return pathRet / ".kaspaz";
    else 
        return pathRet / ".kaspaz" / chainName.symbol();
#endif
}
 ...
 * @returns the full OS-specific data directory including Komodo (i.e. "~/.kaspaz")
 */
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No kaspaz.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override kaspaz.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "kaspazd.pid"));
