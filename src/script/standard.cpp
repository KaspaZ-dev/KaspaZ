...
bool kaspaz_is_vSolutionsFixActive(); // didn't want to bring kaspaz headers here, it's a special case to bypass bad code in Solver() and ExtractDestination() 
    ...
    if (IsCryptoConditionsEnabled()) {
        ...
        if (scriptPubKey.IsPayToCryptoCondition(&ccSubScript, vParams))
        {
            if (scriptPubKey.MayAcceptCryptoCondition())
            {
                ...
                if (!kaspaz_is_vSolutionsFixActive()) 
                {
...
bool ExtractDestination(const CScript& _scriptPubKey, CTxDestination& addressRet, bool returnPubKey)
{
    ...
    {
        if (vSolutions.size() > 1 && !kaspaz_is_vSolutionsFixActive()) // allow this temporarily before the HF; actually this is incorrect to use opdrop's pubkey as the address
        {
