...
//extern std::mutex kaspaz_mutex;  //todo remove

struct kaspaz_event
{
    struct kaspaz_event *related;
    uint16_t len;
    int32_t height;
...
{
public:
    event(kaspaz_event_type t, int32_t height) : type(t), height(height) {}
    virtual ~event() = default;
    kaspaz_event_type type;
    int32_t height;
};
...
{
    event_rewind() : event(kaspaz_event_type::EVENT_REWIND, 0) {}
...
{
    event_notarized() : event(kaspaz_event_type::EVENT_NOTARIZED, 0), notarizedheight(0), MoMdepth(0) {
        memset(this->dest, 0, sizeof(this->dest));
    }
    event_notarized(int32_t ht, const char* _dest) : event(kaspaz_event_type::EVENT_NOTARIZED, ht), notarizedheight(0), MoMdepth(0) {
...
struct kaspaz_ccdataMoM
{
    uint256 MoM;
    int32_t MoMdepth,notarized_height,height,txi;
};

struct kaspaz_ccdata_entry { uint256 MoM; int32_t notarized_height,kmdheight,txi; char symbol[65]; };
struct kaspaz_ccdatapair { int32_t notarized_height,MoMoMoffset; };

struct kaspaz_ccdataMoMoM
{
    uint256 MoMoM;
    int32_t kmdstarti,kmdendi,MoMoMoffset,MoMoMdepth,numpairs,len;
    struct kaspaz_ccdatapair *pairs;
};

struct kaspaz_ccdata
{
    struct kaspaz_ccdata *next,*prev;
    struct kaspaz_ccdataMoM MoMdata;
    uint32_t CCid,len;
    char symbol[65];
};
...
public:
    ...
    uint64_t redeemed;
    uint64_t shorted;
    std::list<std::shared_ptr<kaspaz::event>> events;
