...
void KomodoOceanGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a KaspaZ address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kaspaz: URIs)"));
    ...
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified KaspaZ addresses"));
    ...
    openAction->setStatusTip(tr("Open a kaspaz: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible KaspaZ command-line options").arg(tr(PACKAGE_NAME)));
...
void KomodoOceanGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to KaspaZ network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("KaspaZ"); // default title
