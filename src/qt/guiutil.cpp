...
#include "kaspazaddressvalidator.h"
#include "kaspazunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allowZAddresses)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a KaspaZ address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no kaspaz: URI
    if(!uri.isValid() || uri.scheme() != QString("kaspaz"))
        return false;
...
{
    // Convert kaspaz:// to kaspaz:
    //
    //    Cannot handle this later, because kaspaz:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("kaspaz://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "kaspaz:");
    }
...
{
    QString ret = QString("kaspaz:%1").arg(info.address);
    int paramCount = 0;
...
bool openKomodoConf()
{
    ...
    /* Open kaspaz.conf with the associated application */
...
//    std::string chain = ChainNameFromCommandLine();
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "KaspaZ.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "KaspaZ (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("KaspaZ (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    ...
    CBaseChainParams::Network chain = NetworkIdFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kaspaz.desktop";
    return GetAutostartDir() / strprintf("kaspaz-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        //std::string chain = ChainNameFromCommandLine();
        CBaseChainParams::Network chain = NetworkIdFromCommandLine();
        // Write a kaspaz.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=KaspaZ\n";
        else
            optionFile << strprintf("Name=KaspaZ (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the kaspaz app
...
bool GetStartOnSystemStartup()
{
    CFURLRef kaspazAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (kaspazAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, kaspazAppUrl);

    CFRelease(kaspazAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add kaspaz app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, kaspazAppUrl, nullptr, nullptr);
    }
