...
const QString KOMODO_IPC_PREFIX("kaspaz:");
...
const char* BIP71_MIMETYPE_PAYMENT = "application/kaspaz-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/kaspaz-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/kaspaz-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the kaspaz: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(KOMODO_IPC_PREFIX, Qt::CaseInsensitive)) // kaspaz: URI
        {
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click kaspaz: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start kaspaz: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling kaspaz: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in kaspaz: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(KOMODO_IPC_PREFIX, Qt::CaseInsensitive)) // kaspaz: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseKomodoURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid KaspaZ address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom kaspaz addresses are not supported
