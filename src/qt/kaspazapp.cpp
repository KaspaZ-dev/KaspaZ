...
{
    return QCoreApplication::translate("kaspaz-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. kaspaz_de.qm (shortcut "de" needs to be defined in kaspaz.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. kaspaz_de_DE.qm (shortcut "de_DE" needs to be defined in kaspaz.qrc)
...
static const char* qt_argv = "kaspaz-qt";
...
void KomodoApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // kaspaz: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", KomodoOceanGUI::tr("A fatal error occurred. KaspaZ can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(kaspaz);
    Q_INIT_RESOURCE(kaspaz_locale);
    ...
    if (IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help") || IsArgSet("-version"))
    {
        ...
        LogPrintf("call kaspaz_args.(%s) NOTARY_PUBKEY.(%s)\n",argv[0],NOTARY_PUBKEY.c_str());
    ...
    /// 6. Determine availability of data directory and parse kaspaz.conf
...
//        SelectParams(ChainNameFromCommandLine());
    ...
    // kaspaz: links repeatedly have their payment requests routed to this process:
