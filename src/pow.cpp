...
/* KOMODO_LOADINGBLOCKS moved from here to kaspaz_globals.cpp and made boolean */
...
bool CheckProofOfWork(const CBlockHeader &blkHeader, uint8_t *pubkey33, int32_t height, const Consensus::Params& params)
{
    ...
    if ( height > 34000 && chainName.isKMD() ) // 0 -> non-special notary
    {
        ...
        flag = kaspaz_eligiblenotary(pubkeys,mids,blocktimes,&nonzpkeys,height);
        special2 = kaspaz_is_special(pubkeys,mids,blocktimes,height,pubkey33,blkHeader.nTime);
        if ( notaryid >= 0 )
        {
