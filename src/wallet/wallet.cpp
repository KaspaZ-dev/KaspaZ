        ...
        if (mi != mapWallet.end())
        {
            ...
            if (txin.prevout.n < prev.vout.size())
                if (::IsMine(*this, prev.vout[txin.prevout.n].scriptPubKey) & filter)
                    return prev.vout[txin.prevout.n].nValue; // kaspaz_interest?
        }
    }
    return 0;
}
                    ...
                    if ( !IS_MODE_EXCHANGEWALLET )
                    {
                        ...
                        if ( chainName.isKMD() && chainActive.Tip() != 0 && chainActive.Tip()->nHeight >= 60000 )
                        {
                            if ( pcoin->vout[i].nValue >= 10*COIN )
                            {
                                ...
                                {
                                    kaspaz_accrued_interest(&txheight,&locktime,wtxid,i,0,pcoin->vout[i].nValue,(int32_t)tipindex->nHeight);
                                    interest = kaspaz_interestnew(txheight,pcoin->vout[i].nValue,locktime,tipindex->nTime);
                                } 
                                else 
                                    interest = 0;
                                if ( interest != 0 )
                                ...
                                int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign)
{
    ...
    {
        if ( !kaspaz_hardfork_active((uint32_t)chainActive.Tip()->nTime) )
