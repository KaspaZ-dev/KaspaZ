...
uint32_t GetLatestTimestamp(int32_t height)
{
    ...
    return(kaspaz_heightstamp(height));
}
...
int64_t CCduration(int32_t &numblocks,uint256 txid)
{
    ...
    else if ( (pindex= kaspaz_getblockindex(hashBlock)) == 0 || (txtime= pindex->nTime) == 0 || (txheight= pindex->nHeight) <= 0 )
    {
...
int32_t kaspaz_get_current_height()
{
    if ( KOMODO_NSPV_SUPERLITE )
    {
...
bool kaspaz_txnotarizedconfirmed(uint256 txid)
{
    ...
    if ( KOMODO_NSPV_SUPERLITE )
    {
        ...
        {
            LogPrintf("kaspaz_txnotarizedconfirmed cant find txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if (txheight<=0)
        {
            LogPrintf("kaspaz_txnotarizedconfirmed no txheight.%d for txid %s\n",txheight,txid.ToString().c_str());
            return(0);
        }
        else if (txheight>currentheight)
        {
            LogPrintf("kaspaz_txnotarizedconfirmed backwards heights for txid %s hts.(%d %d)\n",txid.ToString().c_str(),txheight,currentheight);
            return(0);
        }
    ...
    else
    {
        ...
        {
            LogPrintf("kaspaz_txnotarizedconfirmed no hashBlock for txid %s\n",txid.ToString().c_str());
            return(0);
        }
        else if ( (pindex= kaspaz_blockindex(hashBlock)) == 0 || (txheight= pindex->nHeight) <= 0 )
        {
            LogPrintf("kaspaz_txnotarizedconfirmed no txheight.%d %p for txid %s\n",txheight,pindex,txid.ToString().c_str());
            return(0);
        }
    ...
    if ((sp= kaspaz_stateptr(symbol,dest)) != 0 && (notarized=sp->LastNotarizedHeight()) > 0 && txheight > sp->LastNotarizedHeight())  notarized=0;            
#ifdef TESTMODE           
    notarized=0;
#endif //TESTMODE
