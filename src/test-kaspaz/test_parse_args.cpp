...
namespace ParseArgumentsTests {
    ...
    class ParseArgumentsTests: public ::testing::Test {
        ...
        public:
            ...
            void SetUp( ) {
                ...
                STAKED_NOTARY_ID = -1; // should be set via kaspaz_args call in real world
                SelectParams(CBaseChainParams::MAIN); // by default it's a CBaseChainParams::REGTEST, see ./src/test-kaspaz/main.cpp
                chainName = assetchain();

                kaspaz_setactivation(Consensus::NetworkUpgrade::ALWAYS_ACTIVE); // act as UpdateNetworkUpgradeParameters for regtest, to set sapling & overwinter activation height, but for mainnet
            ...
            void TearDown( ) {
                ...
                chainActive.SetTip(nullptr);
                kaspaz_setactivation(Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT);
    ...
    TEST_F(ParseArgumentsTests, ParseCommandLineArgs) {
        ...
        const char program_name[] = "kaspaz-test";
        ...
        for(const auto& pair : mapKnownAssetchains) {
            ...
            ParseParameters(argc, argv.get()); // before calling kaspaz_args -ac_name param should be set in mapArgs
            int64_t oldMAX_MONEY = MAX_MONEY;
            kaspaz_args(argv0Data.get());      // argv0 is passed in try to get ac_name from program suffixes (works for MNZ and BTCH only)
