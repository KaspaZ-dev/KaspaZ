...
#include "kaspaz_bitcoind.h" // kaspaz_verifynotarization
#include "kaspaz_notary.h" // kaspaz_notarized_update
#include "kaspaz_utils.h" // portable_mutex_lock / unlock
#include "kaspaz_kv.h"
...
{
    if (IS_KOMODO_NOTARY)   {
        int32_t ntz_verify = kaspaz_verifynotarization(symbol, ntz.dest, height, ntz.notarizedheight, ntz.blockhash, ntz.desttxid);
        LogPrint("notarisation", "kaspaz_verifynotarization result %d\n", ntz_verify);
    ...
    if (chainName.isSymbol(symbol) || chainName.isKMD() && std::string(symbol) == "KMD" /*special case for KMD*/)
    {
        if (sp != nullptr)
        {
            ...
            kaspaz_notarized_update(sp, height, ntz.notarizedheight, ntz.blockhash, ntz.desttxid, ntz.MoM, ntz.MoMdepth);
        } else {
            LogPrintf("could not update notarisation event: kaspaz_state is null");
        }
    } else {
...
void kaspaz_eventadd_pubkeys(kaspaz_state *sp, const char *symbol, int32_t height, kaspaz::event_pubkeys& pk)
{
    if (sp != nullptr)
    {
        ...
        kaspaz_notarysinit(height, pk.pubkeys, pk.num);
    }
}
...
void kaspaz_eventadd_opreturn( kaspaz_state *sp, const char *symbol, int32_t height, kaspaz::event_opreturn& opret)
{
    if ( sp != nullptr && !chainName.isKMD() )
    {
        ...
        //kaspaz_opreturn(height, opret->value, opret->opret.data(), opret->opret.size(), opret->txid, opret->vout, symbol);
        if ( opret.opret.data()[0] == 'K' && opret.opret.size() != 40 )
        {
            kaspaz_kvupdate(opret.opret.data(), opret.opret.size(), opret.value);
        }
    }
}
 ...
 */
template<class T>
void kaspaz_event_undo(kaspaz_state *sp, T& ev)
{
}

template<>
void kaspaz_event_undo(kaspaz_state* sp, kaspaz::event_kmdheight& ev)
    {
...
void kaspaz_event_rewind(kaspaz_state *sp, const char *symbol, int32_t height)
{
    if ( sp != nullptr )
    {
        ...
        while ( sp->events.size() > 0)
        {
            ...
            if (ev-> height < height)
                    break;
            kaspaz_event_undo(sp, *ev);
            sp->events.pop_back();
        }
    }
}

void kaspaz_setkmdheight(struct kaspaz_state *sp,int32_t kmdheight,uint32_t timestamp)
{
    if ( sp != nullptr )
    {
 ...
 */
void kaspaz_eventadd_kmdheight(struct kaspaz_state *sp, const char *symbol,int32_t height,
        kaspaz::event_kmdheight& kmdht)
{
    if (sp != nullptr)
    {
        if ( kmdht.kheight > 0 ) // height is advancing
        {
            ...
            kaspaz_setkmdheight(sp, kmdht.kheight, kmdht.timestamp);
        }
        else // rewinding
        {
            kaspaz::event_rewind e(height);
            sp->add_event(symbol, height, e);
            kaspaz_event_rewind(sp,symbol,height);
        }
    }
}
