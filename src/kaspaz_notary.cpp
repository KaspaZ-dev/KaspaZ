...
#include "kaspaz.h" // kaspaz_stateupdate()
#include "kaspaz_structs.h" // KOMODO_NOTARIES_HARDCODED
#include "kaspaz_utils.h" // kaspaz_stateptr
#include "kaspaz_bitcoind.h"

//struct knotaries_entry *Pubkeys;  // todo remove

// statics used within this .cpp for caching purposes
static int didinit; // see kaspaz_init
static uint8_t kmd_pubkeys[NUM_KMD_SEASONS][64][33]; // see kaspaz_notaries
static int32_t hwmheight; // highest height ever passed to kaspaz_notariesinit
static int32_t hadnotarization; // used in kaspaz_dpowconfs
static bool didinit_NOTARIES[NUM_KMD_SEASONS]; // kaspaz_notaries()
...
int32_t kaspaz_isnotaryvout(char *coinaddr,uint32_t tiptime) // from ac_private chains only
{
    int32_t season = getacseason(tiptime);
    if ( NOTARY_ADDRESSES[season-1][0][0] == 0 )
    {
        uint8_t pubkeys[64][33];
        kaspaz_notaries(pubkeys,0,tiptime);
    }
 ...
 */
int32_t kaspaz_notaries(uint8_t pubkeys[64][33],int32_t height,uint32_t timestamp)
{
    ...
    if ( chainName.isKMD() )   
        ...
        timestamp = kaspaz_heightstamp(height); // derive the timestamp from the passed-in height
    ...
    std::lock_guard<std::mutex> lock(kaspaz_mutex);
...
int32_t kaspaz_electednotary(int32_t *numnotariesp,uint8_t *pubkey33,int32_t height,uint32_t timestamp)
{
    int32_t i,n; uint8_t pubkeys[64][33];
    n = kaspaz_notaries(pubkeys,height,timestamp);
    *numnotariesp = n;
    for (i=0; i<n; i++)
    {
...
int32_t kaspaz_ratify_threshold(int32_t height,uint64_t signedmask)
{
 ...
 */
void kaspaz_notarysinit(int32_t origheight,uint8_t pubkeys[64][33],int32_t num)
{
    if ( Pubkeys == 0 )
...
int32_t kaspaz_chosennotary(int32_t *notaryidp,int32_t height,uint8_t *pubkey33,uint32_t timestamp)
{
    ...
    *notaryidp = -1;
    ...
    {
        LogPrintf("kaspaz_chosennotary ht.%d illegal\n",height);
        return(-1);
    }
    if ( height >= KOMODO_NOTARIES_HARDCODED || !chainName.isKMD() )
    {
        if ( (*notaryidp= kaspaz_electednotary(&numnotaries,pubkey33,height,timestamp)) >= 0 && numnotaries != 0 )
        {
 ...
 */
const notarized_checkpoint *kaspaz_npptr(int32_t height)
{
    char symbol[KOMODO_ASSETCHAIN_MAXLEN];
    char dest[KOMODO_ASSETCHAIN_MAXLEN]; 

    kaspaz_state *sp = kaspaz_stateptr(symbol, dest);
    if ( sp != nullptr )
    {
 ...
 */
int32_t kaspaz_notarized_height(int32_t *prevMoMheightp,uint256 *hashp,uint256 *txidp)
{
    char symbol[KOMODO_ASSETCHAIN_MAXLEN];
    char dest[KOMODO_ASSETCHAIN_MAXLEN];
...
int32_t kaspaz_dpowconfs(int32_t txheight,int32_t numconfs)
{
    ...
    if ( KOMODO_DPOWCONFS != 0 && txheight > 0 && numconfs > 0 && (sp= kaspaz_stateptr(symbol,dest)) != nullptr )
    {
...
int32_t kaspaz_MoMdata(int32_t *notarized_htp,uint256 *MoMp,uint256 *kmdtxidp,int32_t height,uint256 *MoMoMp,int32_t *MoMoMoffsetp,int32_t *MoMoMdepthp,int32_t *kmdstartip,int32_t *kmdendip)
{
    const notarized_checkpoint *np = kaspaz_npptr(height);
    if ( np != nullptr )
    {
 ...
 * @returns the notarized height
 */
int32_t kaspaz_notarizeddata(int32_t nHeight,uint256 *notarized_hashp,uint256 *notarized_desttxidp)
{
    char symbol[KOMODO_ASSETCHAIN_MAXLEN];
    char dest[KOMODO_ASSETCHAIN_MAXLEN];
...
/***
 * Add a notarized checkpoint to the kaspaz_state
 * @param[in] sp the kaspaz_state to add to
 ...
 */
void kaspaz_notarized_update(struct kaspaz_state *sp,int32_t nHeight,int32_t notarized_height,
        uint256 notarized_hash,uint256 notarized_desttxid,uint256 MoM,int32_t MoMdepth)
{
    if ( notarized_height >= nHeight )
    {
        LogPrintf("kaspaz_notarized_update REJECT notarized_height %d > %d nHeight\n",notarized_height,nHeight);
        return;
    }
 ...
 */
void kaspaz_init(int32_t height)
{
    uint256 zero; 
    uint8_t pubkeys[64][33];
    ...
    if ( !didinit )
    {
        ...
        if ( height >= 0 )
        {
            ...
            kaspaz_notarysinit(0,pubkeys,count);
        }
        didinit = true;
        kaspaz_stateupdate(0,0,0,0,zero,0,0,0,0,0,0,0,0,zero,0);
    }
}

/****
 * A nasty hack to reset statics in this file.
 * Do not put this in kaspaz_notary.h, as it should only be used for testing
 */
void kaspaz_notaries_uninit()
{
    didinit = 0;
    hwmheight = 0;
    hadnotarization = 0;
