...
{
    char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN]; struct kaspaz_state *sp;
    if ( (sp= kaspaz_stateptr(symbol,dest)) != 0 )
    ...
    try
    {
        if ( (func= fgetc(fp)) != EOF )
        {
            ...
            if ( fread(&ht,1,sizeof(ht),fp) != sizeof(ht) )
                throw kaspaz::parse_error("Unable to read height from file");
            if ( func == 'P' )
            {
                kaspaz::event_pubkeys pk(fp, ht);
                if ( (KOMODO_EXTERNAL_NOTARIES && matched ) || (strcmp(symbol,"KMD") == 0 && !KOMODO_EXTERNAL_NOTARIES) )
                {
                    kaspaz_eventadd_pubkeys(sp, symbol, ht, pk);
                }
            }
            else if ( func == 'N' || func == 'M' )
            {
                kaspaz::event_notarized evt(fp, ht, dest, func == 'M');
                kaspaz_eventadd_notarized(sp, symbol, ht, evt);
            }
            else if ( func == 'U' ) // deprecated
            {
                kaspaz::event_u evt(fp, ht);
            }
            else if ( func == 'K' || func == 'T')
            {
                kaspaz::event_kmdheight evt(fp, ht, func == 'T');
                kaspaz_eventadd_kmdheight(sp, symbol, ht, evt);
            }
            else if ( func == 'R' )
            {
                kaspaz::event_opreturn evt(fp, ht);
                // check for oversized opret
                if ( evt.opret.size() < 16384*4 )
                    kaspaz_eventadd_opreturn(sp, symbol, ht, evt);
            }
            else if ( func == 'D' )
            {
            ...
            {
                kaspaz::event_pricefeed evt(fp, ht);
                kaspaz_eventadd_pricefeed(sp, symbol, ht, evt);
            }
            else if ( func == 'B' ) {
                // can be written but not processed on read
            }
            else {
                throw kaspaz::parse_error("Unable to parse state file: unknown event");
            }
        } // retrieved the func
    }
    catch(const kaspaz::parse_error& pe)
    {
    ...
    try
    {
        ...
        if ( fpos < datalen )
        {
            ...
            if ( mem_read(ht, filedata, fpos, datalen) != sizeof(ht) )
                throw kaspaz::parse_error("Unable to parse height from file data");
            if ( func == 'P' )
            {
                kaspaz::event_pubkeys pk(filedata, fpos, datalen, ht);
                if ( (KOMODO_EXTERNAL_NOTARIES && matched ) || (strcmp(symbol,"KMD") == 0 && !KOMODO_EXTERNAL_NOTARIES) )
                {
                    kaspaz_eventadd_pubkeys(sp, symbol, ht, pk);
                }
            ...
            {
                kaspaz::event_notarized ntz(filedata, fpos, datalen, ht, dest, func == 'M');
                kaspaz_eventadd_notarized(sp, symbol, ht, ntz);
            }
            else if ( func == 'U' ) // deprecated
            {
                kaspaz::event_u u(filedata, fpos, datalen, ht);
            }
            else if ( func == 'K' || func == 'T' )
            {
                kaspaz::event_kmdheight kmd_ht(filedata, fpos, datalen, ht, func == 'T');
                kaspaz_eventadd_kmdheight(sp, symbol, ht, kmd_ht);
            }
            else if ( func == 'R' )
            {
                kaspaz::event_opreturn opret(filedata, fpos, datalen, ht);
                kaspaz_eventadd_opreturn(sp, symbol, ht, opret);
            }
            else if ( func == 'D' )
            {
            ...
            {
                kaspaz::event_pricefeed pf(filedata, fpos, datalen, ht);
                kaspaz_eventadd_pricefeed(sp, symbol, ht, pf);
            }
            else if ( func == 'B' ) {
                // can be written but not processed on read
            }
            else {
                throw kaspaz::parse_error("Unable to parse file data: unknown event");
            }
            *fposp = fpos;
        }
    }
...
void kaspaz_stateupdate(int32_t height,uint8_t notarypubs[][33],uint8_t numnotaries,
        ...
        uint8_t *opretbuf,uint16_t opretlen,uint16_t vout,uint256 MoM,int32_t MoMdepth)
{
    ...
    if ( (sp= kaspaz_stateptr(symbol,dest)) == 0 )
    {
        ...
        LogPrintf("[%s] no kaspaz_stateptr\n",chainName.symbol().c_str());
        return;
    }
    if ( fp == 0 )
    {
        kaspaz_statefname(fname, chainName.symbol().c_str(), KOMODO_STATE_FILENAME);
        if ( (fp= fopen(fname,"rb+")) != nullptr )
        {
            if ( kaspaz_faststateinit(sp, fname, symbol, dest) )
                fseek(fp,0,SEEK_END);
            else
            {
                // unable to use faststateinit, so try again only slower
                LogPrintf("kaspaz_faststateinit retval.-1\n");
                while (!ShutdownRequested() && kaspaz_parsestatefile(sp,fp,symbol,dest) >= 0)
                    ;
            }
            LogPrintf("kaspaz read last notarised height %d from %s\n", sp->LastNotarizedHeight(), KOMODO_STATE_FILENAME);
        } 
        else 
    ...
    if ( fp != 0 ) // write out funcid, height, other fields, call side effect function
    {
        if ( KMDheight != 0 )
        {
            kaspaz::event_kmdheight kmd_ht(height);
            kmd_ht.kheight = KMDheight;
            kmd_ht.timestamp = KMDtimestamp;
            write_event(kmd_ht, fp);
            kaspaz_eventadd_kmdheight(sp,symbol,height,kmd_ht);
        }
        else if ( opretbuf != 0 && opretlen > 0 )
        {
            kaspaz::event_opreturn evt(height);
            evt.txid = txhash;
            evt.vout = vout;
            ...
            write_event(evt, fp);
            kaspaz_eventadd_opreturn(sp,symbol,height,evt);
        }
        else if ( notarypubs != 0 && numnotaries > 0 )
        {
            kaspaz::event_pubkeys pk(height);
            pk.num = numnotaries;
            memcpy(pk.pubkeys, notarypubs, 33 * 64);
            write_event(pk, fp);
            kaspaz_eventadd_pubkeys(sp,symbol,height,pk);
        }
        /* TODO: why is this removed in jmj_event_fix3?
        else if ( voutmask != 0 && numvouts > 0 )
        {
            kaspaz::event_u evt(height);
            evt.n = numvouts;
            evt.nid = notaryid;
        ...
        else if ( pvals != 0 && numpvals > 0 )
        {
            ...
            if ( nonz >= 32 )
            {
                kaspaz::event_pricefeed evt(height);
                evt.num = numpvals;
                for( uint8_t i = 0; i < evt.num; ++i)
                    evt.prices[i] = pvals[i];
                write_event(evt, fp);
                kaspaz_eventadd_pricefeed(sp,symbol,height,evt);
            }
        }
        else if ( height != 0 )
        {
            if ( sp != nullptr )
            {
                kaspaz::event_notarized evt(height, dest);
                ...
                write_event(evt, fp);
                kaspaz_eventadd_notarized(sp,symbol,height,evt);
            }
        }
        fflush(fp);
    }
...
int32_t kaspaz_validate_chain(uint256 srchash,int32_t notarized_height)
{
    static int32_t last_rewind; int32_t rewindtarget; CBlockIndex *pindex; struct kaspaz_state *sp; char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN];
    if ( (sp= kaspaz_stateptr(symbol,dest)) == 0 )
        return(0);
    if ( IsInitialBlockDownload() == 0 && ((pindex= kaspaz_getblockindex(srchash)) == 0 || pindex->nHeight != notarized_height) )
    {
...
namespace {
    ...
    std::string GetChainNameFromScript(uint8_t* scriptbuf, int32_t scriptlen, size_t offsetInScript)
    {
        ...
        const size_t maxChainName = sizeof(kaspaz_ccdata::symbol);
...
int32_t kaspaz_voutupdate(bool fJustCheck,int32_t *isratificationp,int32_t notaryid,uint8_t *scriptbuf,
        ...
        uint64_t signedmask,uint32_t timestamp)
{
    ...
    int32_t opretlen,nid,offset,k,MoMdepth,matched,len = 0; uint256 MoM,srchash,desttxid; uint8_t crypto777[33]; struct kaspaz_state *sp; char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN];
    if ( (sp= kaspaz_stateptr(symbol,dest)) == 0 )
        return(-1);
    if ( scriptlen == 35 && scriptbuf[0] == 33 && scriptbuf[34] == 0xac )
    {
        ...
        else if ( kaspaz_chosennotary(&nid,height,scriptbuf + 1,timestamp) >= 0 )
        {
    ...
    {
        struct kaspaz_ccdata ccdata; struct kaspaz_ccdataMoMoM MoMoMdata;
        ...
        if ( j == 1 && opretlen >= len+offset-opoffset )
        {
            ...
            if ( matched != 0 )
                validated = kaspaz_validate_chain(srchash,*notarizedheightp);
            else validated = 1;
            ...
            if ( notarized != 0 && validated != 0 )
            {
                    ...
                    if ( len+sizeof(ccdata.CCid)-opoffset <= opretlen )
                    {
                        ...
                        if ( !chainName.isKMD() )
                        {
                            ...
                            if ( len+48-opoffset <= opretlen && chainName.isSymbol(ccdata.symbol) )
                            {
                                ...
                                {
                                    MoMoMdata.pairs = (struct kaspaz_ccdatapair *)calloc(MoMoMdata.numpairs,sizeof(*MoMoMdata.pairs));
                ...
                if ( matched != 0 && *notarizedheightp > sp->LastNotarizedHeight() && *notarizedheightp < height )
                {
                    ...
                    kaspaz_stateupdate(height,0,0,0,zero,0,0,0,0,0,0,0,0,sp->LastNotarizedMoM(),sp->LastNotarizedMoMDepth());
                    ...
                    if ( chainName.isKMD() )
                    {
                        if ( signedfp == 0 )
                        {
                            ...
                            kaspaz_statefname(fname,chainName.symbol().c_str(),(char *)"signedmasks");
...
int32_t kaspaz_notarycmp(uint8_t *scriptPubKey,int32_t scriptlen,uint8_t pubkeys[64][33],int32_t numnotaries,uint8_t rmd160[20])
{
    int32_t i;
...
*/
/* 
    JMJ: Moved hwmheight out of kaspaz_connectblock to allow testing. 
    Adjusting this should only be done by kaspaz_connectblock or a unit test
*/
static int32_t hwmheight;

void adjust_hwmheight(int32_t newHeight) { hwmheight = newHeight; }
void clear_fp_stateupdate() { fp = nullptr; } // tests should clear fp, before new call(s) to kaspaz_stateupdate if datadir is changed
...
int32_t kaspaz_connectblock(bool fJustCheck, CBlockIndex *pindex,CBlock& block)
{
    ...
    uint64_t signedmask,voutmask; char symbol[KOMODO_ASSETCHAIN_MAXLEN],dest[KOMODO_ASSETCHAIN_MAXLEN]; struct kaspaz_state *sp;
    ...
    if ( pindex == 0 )
    {
        LogPrintf("kaspaz_connectblock null pindex\n");
        return(0);
    }
    memset(&zero,0,sizeof(zero));
    kaspaz_init(pindex->nHeight);
    KOMODO_INITDONE = (uint32_t)time(NULL);
    if ( (sp= kaspaz_stateptr(symbol,dest)) == 0 )
    {
        LogPrintf("unexpected null kaspazstateptr.[%s]\n",chainName.symbol().c_str());
        return(0);
    }
    ...
    numnotaries = kaspaz_notaries(pubkeys,pindex->nHeight,pindex->GetBlockTime());
    ...
    else
    {
        if ( pindex->nHeight != hwmheight )
        {
            ...
            kaspaz_purge_ccdata((int32_t)pindex->nHeight);
            hwmheight = pindex->nHeight;
        }
        if (!fJustCheck)
        {
            kaspaz_event_rewind(sp,symbol,pindex->nHeight);
            kaspaz_stateupdate(pindex->nHeight,0,0,0,zero,0,0,-pindex->nHeight,pindex->nTime,0,0,0,0,zero,0);
        }
    }
    ...
    if ( pindex != 0 )
    {
        ...
        for (i=0; i<txn_count; i++)
        {
            ...
            for (j=0; j<numvins; j++)
            {
                ...
                {
                    if ( (k= kaspaz_notarycmp(scriptPubKey,scriptlen,pubkeys,numnotaries,rmd160)) >= 0 )
                        signedmask |= (1LL << k);
            ...
            for (j=0; j<numvouts; j++)
            {
                ...
                if ( len >= sizeof(uint32_t) && len <= sizeof(scriptbuf) )
                {
                    ...
                    notaryid = kaspaz_voutupdate(fJustCheck,&isratification,notaryid,scriptbuf,len,height,txhash,i,j,&voutmask,&specialtx,&notarizedheight,(uint64_t)block.vtx[i].vout[j].nValue,notarized,signedmask,(uint32_t)chainActive.Tip()->GetBlockTime());
                ...
                if ( specialtx != 0 && isratification != 0 && numvouts > 2 )
                {
                    if (chainName.isKMD() && height < 100000)
                    {
                        ...
                        if ( ((signedmask & 1) != 0 && numvalid >= KOMODO_MINRATIFY) || bitweight(signedmask) > (numnotaries/3) )
                        {
                            ...
                            kaspaz_stateupdate(height,pubkeys,numvalid,0,txhash,0,0,0,0,0,0,0,0,zero,0);
        ...
        if ( !fJustCheck && pindex->nHeight == hwmheight )
            kaspaz_stateupdate(height,0,0,0,zero,0,0,height,(uint32_t)pindex->nTime,0,0,0,0,zero,0);
    } 
    else 
        { LogPrintf("kaspaz_connectblock: unexpected null pindex\n"); return(0); }
    if (fJustCheck)
    {
        ...
        LogPrintf("kaspaz_connectblock: unxexpected behaviour when fJustCheck == true, report blackjok3rtt plz ! \n");
