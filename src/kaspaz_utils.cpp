...
/*int32_t kaspaz_baseid(const char *origbase)
{
...
uint64_t kaspaz_block_prg(uint32_t nHeight)
{
...
int64_t kaspaz_block_unlocktime(uint32_t nHeight)
{
    ...
    {
        unlocktime = kaspaz_block_prg(nHeight) % (ASSETCHAINS_TIMEUNLOCKTO - ASSETCHAINS_TIMEUNLOCKFROM);
 ...
 * @return the RPC port
 */
uint16_t _kaspaz_userpass(char *username,char *password,FILE *fp)
{
    uint16_t port = 0;
...
void kaspaz_statefname(char *fname, const char *symbol, const char *str)
{
    int32_t n,len;
    ...
    if ( (n= (int32_t)chainName.symbol().size()) != 0 )
    {
        ...
        if ( !mapArgs.count("-datadir") && strcmp(chainName.symbol().c_str(),&fname[len - n]) == 0 )
            fname[len - n] = 0;
        else if(mapArgs.count("-datadir")) LogPrintf("DEBUG - kaspaz_utils:1363: custom datadir\n");
        else
        {
...
void kaspaz_configfile(const char *symbol,uint16_t rpcport)
{
    ...
    if ( symbol != 0 && rpcport != 0 )
    {
        ...
        {
            _kaspaz_userpass(myusername,mypassword,fp);
    ...
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        if ( (kmdport= _kaspaz_userpass(username,password,fp)) != 0 )
            KMD_PORT = kmdport;
...
uint16_t kaspaz_userpass(char *userpass,const char *symbol)
{
...
#else
        sprintf(confname,"kaspaz.conf");
#endif
    }
    else if(!mapArgs.count("-conf")) {
        sprintf(confname,"%s.conf",symbol);
        kaspaz_statefname(fname,symbol,confname);
    } else sprintf(fname,"%s",GetDataDir().string().c_str());
    
    if ( (fp= fopen(fname,"rb")) != 0 )
    {
        port = _kaspaz_userpass(username,password,fp);
 ...
 */
uint32_t kaspaz_assetmagic(const char *symbol,uint64_t supply,uint8_t *extraptr,int32_t extralen)
{
    uint8_t buf[512]; 
    uint32_t crc0=0; 
    int32_t len = 0; 
 ...
 */
uint16_t kaspaz_assetport(uint32_t magic,int32_t extralen)
{
    if ( magic == 0x8de4eef9 )
        return 7770;
    else if ( extralen == 0 )
 ...
 */
uint16_t kaspaz_port(const char *symbol,uint64_t supply,uint32_t *magicp,uint8_t *extraptr,int32_t extralen)
{
    if ( symbol == 0 || symbol[0] == 0 || strcmp("KMD",symbol) == 0 )
    {
        *magicp = 0x8de4eef9;
    ...
    *magicp = kaspaz_assetmagic(symbol,supply,extraptr,extralen);
    return kaspaz_assetport(*magicp,extralen);
}


int32_t kaspaz_whoami(char *pubkeystr,int32_t height,uint32_t timestamp)
{
    int32_t i,notaryid;
    for (i=0; i<33; i++)
        sprintf(&pubkeystr[i<<1],"%02x",NOTARY_PUBKEY33[i]);
    pubkeystr[66] = 0;
    kaspaz_chosennotary(&notaryid,height,NOTARY_PUBKEY33,timestamp);
    return(notaryid);
}
...
uint64_t kaspaz_current_supply(uint32_t nHeight)
{
    uint64_t cur_money;
    int32_t baseid;
...
uint64_t kaspaz_max_money()
{
    return kaspaz_current_supply(10000000);
}

uint64_t kaspaz_ac_block_subsidy(int nHeight)
{
...
int8_t equihash_params_possible(uint64_t n, uint64_t k)
{
    ...
    *  EhOptimisedSolve
    *  EhIsValidSolution
    * Alternatively change ASSETCHAINS_N and ASSETCHAINS_K in kaspaz_nk.h for fast testing.
    */
    if ( k == 9 && (n == 200 || n == 210) )
        return(0);
...
#else
    std::string filename = "kaspaz.conf";
#endif
...
void kaspaz_args(char *argv0)
{
    ...
    SoftSetArg("-ac_name", std::string("KASZ"));
    SoftSetArg("-ac_blocktime", std::string("45"));
    SoftSetArg("-ac_reward", std::string("2250000000000"));
    SoftSetArg("-ac_decay", std::string("0.943874"));
    SoftSetArg("-ac_halving", std::string("58445"));
    SoftSetArg("-ac_adaptivepow", std::string("1"));
    SoftSetArg("-ac_end", std::string("35040000"));
    SoftSetArg("-ac_sapling", std::string("61"));
    ...
    if ( !chainName.isKMD() )
    {
        ...
        MAX_MONEY = kaspaz_max_money();
        int32_t baseid;
        if ( (baseid = kaspaz_baseid(chainName.symbol().c_str())) >= 0 && baseid < 32 )
        {
        ...
        uint16_t tmpport = kaspaz_port(chainName.symbol().c_str(),ASSETCHAINS_SUPPLY,&ASSETCHAINS_MAGIC,extraptr,extralen);
        if ( GetArg("-port",0) != 0 )
        {
        ...
        if ( !chainName.isKMD() )
        {
            ...
            uint16_t port;
            if ( (port= kaspaz_userpass(ASSETCHAINS_USERPASS,chainName.symbol().c_str())) != 0 )
                ASSETCHAINS_RPCPORT = port;
            else kaspaz_configfile(chainName.symbol().c_str(),ASSETCHAINS_P2PPORT + 1);
 ...
 */
void kaspaz_nameset(char *symbol,char *dest,const char *source)
{
...
/****
 * @brief get the right kaspaz_state
 * @param[in] base what to search for (nullptr == "KMD")
 * @returns the correct kaspaz_state object
 */
kaspaz_state *kaspaz_stateptrget(char *base)
{
    // "KMD" case
 ...
 * @returns the kaspaz_state object for symbol
 */
kaspaz_state *kaspaz_stateptr(char *symbol,char *dest)
{
    int32_t baseid;
    kaspaz_nameset(symbol,dest,chainName.symbol().c_str());
    return(kaspaz_stateptrget(symbol));
}
...
bool kaspaz_is_vSolutionsFixActive()
{
    return GetLatestTimestamp(kaspaz_currentheight()) > nS5Timestamp;
}
