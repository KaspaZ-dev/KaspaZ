...
void TxToJSONExpanded(const CTransaction& tx, const uint256 hashBlock, UniValue& entry, int nHeight = 0, int nConfirmations = 0, int nBlockTime = 0)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        if ( chainName.isKMD() && pindex != 0 && tx.nLockTime >= 500000000 && (tipindex= chainActive.Tip()) != 0 )
        {
            ...
            interest = kaspaz_accrued_interest(&txheight,&locktime,tx.GetHash(),i,0,txout.nValue,(int32_t)tipindex->nHeight);
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (nConfirmations > 0) {
            ...
            entry.push_back(Pair("confirmations", kaspaz_dpowconfs(nHeight,nConfirmations)));
    ...
    if (!hashBlock.IsNull()) {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            ...
            if (chainActive.Contains(pindex)) {
                ...
                entry.push_back(Pair("confirmations", kaspaz_dpowconfs(pindex->nHeight,1 + chainActive.Height() - pindex->nHeight)));
...
UniValue createrawtransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the kaspaz address or script (in hex), the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"RTZMZHDFSTFQst8XmX2dR4DaH87cEUs3gC\"   (string) kaspaz address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
