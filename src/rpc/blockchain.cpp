...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("confirmations", kaspaz_dpowconfs(blockindex->nHeight,confirmations)));
    ...
    result.push_back(Pair("segid", (int)kaspaz_segid(0,blockindex->nHeight)));
...
/*uint256 _kaspaz_getblockhash(int32_t nHeight)
{
    uint256 hash;
    LOCK(cs_main);
    if ( nHeight >= 0 && nHeight <= chainActive.Height() )
...
UniValue kvsearch(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (keylen= (int32_t)strlen(params[0].get_str().c_str())) > 0 )
    {
        ...
        if ( keylen < sizeof(key) )
        {
            ...
            if ( (valuesize= kaspaz_kvsearch(&refpubkey,chainActive.Tip()->nHeight,&flags,&height,value,key,keylen)) >= 0 )
            {
...
UniValue minerids(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= kaspaz_minerids(minerids,height,(int32_t)(sizeof(minerids)/sizeof(*minerids)))) > 0 )
    {
        memset(tally,0,sizeof(tally));
        numnotaries = kaspaz_notaries(pubkeys,height,timestamp);
        if ( numnotaries > 0 )
        {
            for (i=0; i<n; i++)
            {
...
UniValue notaries(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if ( (n= kaspaz_notaries(pubkeys,height,timestamp)) > 0 )
    {
        for (i=0; i<n; i++)
        {
            UniValue item(UniValue::VOBJ);
...
UniValue gettxout(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "        \"kaspazaddress\"        (string) Komodo address\n"
            "        ,...\n"
            "     ]\n"
            "  },\n"
    ...
    if ((unsigned int)coins.nHeight == MEMPOOL_HEIGHT) {
        ...
        ret.push_back(Pair("confirmations", kaspaz_dpowconfs(coins.nHeight,pindex->nHeight - coins.nHeight + 1)));
    ...
    if ( (interest= kaspaz_accrued_interest(&txheight,&locktime,hash,n,coins.nHeight,coins.vout[n].nValue,(int32_t)pindex->nHeight)) != 0 )
