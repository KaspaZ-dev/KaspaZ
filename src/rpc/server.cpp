...
{
    if ( chainName.isKMD() ) {
        return "> kaspaz-cli " + methodname + " " + args + "\n";
    } else {
        return "> kaspaz-cli -ac_name=" + strprintf("%s", chainName.symbol().c_str()) + " " + methodname + " " + args + "\n";
    }
}
...
{
    string daemon = "kaspazd";
    string ticker = chainName.isKMD() ? "kaspaz" : chainName.symbol();
