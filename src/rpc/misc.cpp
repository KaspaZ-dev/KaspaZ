...
bool kaspaz_txnotarizedconfirmed(uint256 txid);
...
UniValue getiguanajson(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    // get the min sigs .. this always rounds UP so min sigs in iguana is +1 min sigs in kaspazd, due to some rounding error.
    int minsigs;
...
//#endif
    ...
    obj.push_back(Pair("errors",        GetWarnings("statusbar")));
     if ( NOTARY_PUBKEY33[0] != 0 ) {
        ...
        } else if( (notaryid= kaspaz_whoami(pubkeystr,(int32_t)chainActive.Tip()->nHeight,kaspaz_chainactive_timestamp())) >= 0 )  {
            obj.push_back(Pair("notaryid",        notaryid));
...
UniValue coinsupply(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    if (height >= 0 && height <= currentHeight) {
        if ( (supply= kaspaz_coinsupply(&zfunds,&sproutfunds,height)) > 0 )
        {
            ...
            if ( ASSETCHAINS_BLOCKTIME > 0 )
            {
                ...
                {
                    supply1 = kaspaz_coinsupply(&zf1,&sf1,height - blocks_per_year/12);
                    supply3 = kaspaz_coinsupply(&zf3,&sf3,height - blocks_per_year/4);
                    supply12 = kaspaz_coinsupply(&zf12,&sf12,height - blocks_per_year);
...
UniValue validateaddress(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"kaspazaddress\"\n"
            "\nReturn information about the given Komodo address.\n"
            "\nArguments:\n"
            "1. \"kaspazaddress\"     (string, required) The Komodo address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"kaspazaddress\",   (string) The Komodo address validated\n"
    ...
    if (isValid)
    {
        ...
        ret.push_back(Pair("segid", (int32_t)kaspaz_segid32((char *)params[0].get_str().c_str()) & 0x3f));
#ifdef ENABLE_WALLET
...
UniValue verifymessage(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"kaspazaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"kaspazaddress\"    (string, required) The Komodo address to use for the signature.\n"
...
UniValue getnotarypayinfo(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarycount = kaspaz_notaries(notarypubkeys, height, chainActive[height]->GetBlockTime());
    NotaryPay = kaspaz_notarypayamount(height, notarycount)*notarycount;
...
UniValue txnotarizedconfirmed(const UniValue& params, bool fHelp, const CPubKey& mypk)
{
    ...
    notarizedconfirmed=kaspaz_txnotarizedconfirmed(txid);
    UniValue result(UniValue::VOBJ);
