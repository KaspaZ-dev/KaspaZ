...
#include "kaspaz_extern_globals.h"
...
UniValue kaspaz_snapshot(int top)
{
    LOCK(cs_main);
    int64_t total = -1;
...
bool kaspaz_snapshot2(std::map <std::string, CAmount> &addressAmounts)
{
...
bool kaspaz_dailysnapshot(int32_t height)
{
    int reorglimit = 100; 
    ...
    // Under heavy reorg attack, its possible `kaspaz_notarized_height` can return a height that can't be found on chain sync.
    ...
    else 
    {
        ...
        notarized_height = kaspaz_notarized_height(&prevMoMheight,&notarized_hash,&notarized_desttxid);
    ...
    if ( !kaspaz_snapshot2(addressAmounts) )
        return false;
    ...
    for (int32_t n = height; n > undo_height; n--) 
    {
        ...
        if ( (pindex= kaspaz_chainactive(n)) == 0 || kaspaz_blockload(block, pindex) != 0 ) 
            return false;
...
bool IsFinalTx(const CTransaction &tx, int nBlockHeight, int64_t nBlockTime)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        if ( !kaspaz_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
        ...
        else if ( kaspaz_hardfork_active(nBlockTime) && txin.nSequence == 0xfffffffe &&
            (
                ((int64_t)tx.nLockTime >= LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockTime) ||
                ((int64_t)tx.nLockTime <  LOCKTIME_THRESHOLD && (int64_t)tx.nLockTime <= nBlockHeight))
            )
...
bool ContextualCheckCoinbaseTransaction(int32_t slowflag,const CBlock *block,CBlockIndex * const previndex,const CTransaction& tx, const int nHeight,int32_t validateprices)
{
    ...
    if (((uint64_t)(tx.GetValueOut()) >= ASSETCHAINS_TIMELOCKGTE) || (kaspaz_ac_block_subsidy(nHeight) >= ASSETCHAINS_TIMELOCKGTE))
    {
            ...
            if (tx.vout[1].scriptPubKey.GetOp2(it, op, &opretData))
            {
                if (opretData.size() > 0 && opretData.data()[0] == OPRETTYPE_TIMELOCK)
                {
                    ...
                    if (CScriptID(opretScript) == scriptHash &&
                        ...
                        kaspaz_block_unlocktime(nHeight) == unlocktime)
                    {
                        return(true);
                    }
                }
                      ...
                      libzcash::ProofVerifier& verifier,int32_t txIndex, int32_t numTxs)
{
    if ( chainName.isKMD() )
    {
        ...
        if ( *(int32_t *)&array[0] == 0 )
            numbanned = kaspaz_bannedset(&indallvouts,array,(int32_t)(sizeof(array)/sizeof(*array)));

        for (size_t j=0; j<tx.vin.size(); j++) // for every tx.vin
        {
            for (int32_t k=0; k<numbanned; k++) // go through the array of banned txids
            {
                if ( tx.vin[j].prevout.hash == array[k] && kaspaz_checkvout(tx.vin[j].prevout.n,k,indallvouts) != 0 ) //(tx.vin[j].prevout.n == 1 || k >= indallvouts) )
                {
    ...
    uint256 merkleroot;
    if ( ASSETCHAINS_STAKED != 0 && kaspaz_newStakerActive(0, tiptime) != 0 && tx.vout.size() == 2 && DecodeStakingOpRet(tx.vout[1].scriptPubKey, merkleroot) != 0 )
    {
...
bool CheckTransactionWithoutProofVerification(uint32_t tiptime,const CTransaction& tx, CValidationState &state)
{
    ...
    int32_t invalid_private_taddr=0,z_z=0,z_t=0,t_z=0,acpublic = kaspaz_acpublic(tiptime), current_season = getacseason(tiptime);
    /**
     * Previously:
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout)
    {
        ...
        if ( ASSETCHAINS_PRIVATE != 0 )
        {
            ...
            if (iscoinbase == 0 && txout.nValue > 0)
            {
                ...
                if ( kaspaz_isnotaryvout(destaddr,tiptime) == 0 )
                {
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,bool* pfMissingInputs, bool fRejectAbsurdFee, int dosLevel)
{
    ...
    if ( chainName.isKMD() && chainActive.Tip() != nullptr
            && !kaspaz_validate_interest(tx,chainActive.Tip()->nHeight+1, chainActive.Tip()->GetMedianTimePast() + 777))
    {
        return state.DoS(0, error("%s: kaspaz_validate_interest failed txid.%s", __func__, tx.GetHash().ToString()), REJECT_INVALID, "kaspaz-interest-invalid");
    }
...
bool ReadBlockFromDisk(int32_t height,CBlock& block, const CDiskBlockPos& pos,bool checkPOW)
{
    ...
    if ( 0 && checkPOW != 0 )
    {
        kaspaz_block2pubkey33(pubkey33,(CBlock *)&block);
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    if (chainName.isKMD()) {
        ...
        return kaspaz_ac_block_subsidy(nHeight);
    }
}
    ...
    bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, const Consensus::Params& consensusParams)
    {
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                ...
                {
                    uint64_t unlockTime = kaspaz_block_unlocktime(coins->nHeight);
            ...
            if ( chainName.isKMD() && nSpendHeight > 60000 )
            {
                if ( coins->vout[prevout.n].nValue >= 10*COIN )
                {
                    ...
                    if ( (interest= kaspaz_accrued_interest(&txheight,&locktime,prevout.hash,prevout.n,0,coins->vout[prevout.n].nValue,(int32_t)nSpendHeight-1)) != 0 )
                    {
                        nValueIn += interest;
                    }
                }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    bool fClean = true;
    //kaspaz_disconnect(pindex,block); does nothing?
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck,bool fCheckPOW)
{
    ...
    if ( ASSETCHAINS_NOTARY_PAY[0] != 0 && pindex->nHeight > 10 )
    {
        ...
        int32_t notarisationTx = kaspaz_connectblock(true,pindex,*(CBlock *)&block);  
        ...
        if ( notarisationTx == 1 )
        {
            ...
            notarypaycheque = kaspaz_checknotarypay((CBlock *)&block,(int32_t)pindex->nHeight);
    ...
    */
    ...
    if ( ASSETCHAINS_STAKED != 0 && fCheckPOW && kaspaz_checkPOW(blockReward+stakeTxValue-notarypaycheque,1,(CBlock *)&block,pindex->nHeight) < 0 ) 
        return state.DoS(100, error("ConnectBlock: ac_staked chain failed slow kaspaz_checkPOW"),REJECT_INVALID, "failed-slow_checkPOW");
    ...
    if ( ASSETCHAINS_COMMISSION != 0 || ASSETCHAINS_FOUNDERS_REWARD != 0 ) //ASSETCHAINS_OVERRIDE_PUBKEY33[0] != 0 &&
    {
        ...
        if ( (checktoshis= kaspaz_commission((CBlock *)&block,(int32_t)pindex->nHeight)) != 0 )
        {
    ...
    //FlushStateToDisk();
    kaspaz_connectblock(false,pindex,*(CBlock *)&block);  // dPoW state update.
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    {
        int32_t longestchain = kaspaz_longestchain();
...
bool static DisconnectTip(CValidationState &state, bool fBare = false) {
    ...
    //if ( ASSETCHAINS_SYMBOL[0] != 0 || pindexDelete->nHeight > 1400000 )
    {
        ...
        notarizedht = kaspaz_notarized_height(&prevMoMheight,&notarizedhash,&txid);
    ...
    if (!fBare) {
        ...
        for (int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if (tx.IsCoinBase() || (i == block.vtx.size()-1 && kaspaz_newStakerActive(0, pindexDelete->nTime) == 0 && kaspaz_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0) || !AcceptToMemoryPool(mempool, stateDummy, tx, false, NULL))
            {
    ...
    {
        CTransaction &tx = block.vtx[i];
        //if ((i == (block.vtx.size() - 1)) && ((ASSETCHAINS_STAKED != 0 && (kaspaz_isPoS((CBlock *)&block) != 0))))
        if ( kaspaz_newStakerActive(0, pindexDelete->nTime) == 0 && i == block.vtx.size()-1 && kaspaz_isPoS((CBlock *)&block,pindexDelete->nHeight,0) != 0 )
        {
#ifdef ENABLE_WALLET
...
int32_t kaspaz_activate_sapling(CBlockIndex *pindex)
{
    ...
    if ( blocktime > KOMODO_SAPLING_ACTIVATION ) // find the earliest transition
    {
        while ( (prev= pindex->pprev) != 0 )
        {
            ...
            if ( prevht+1 != height )
            {
                LogPrintf("kaspaz_activate_sapling: unexpected non-contiguous ht %d vs %d\n",prevht,height);
                return(0);
            }
    ...
    if ( activation != 0 )
    {
        kaspaz_setactivation(activation);
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    /*if ( KOMODO_INSYNC != 0 ) //ASSETCHAINS_SYMBOL[0] == 0 &&
        kaspaz_broadcast(pblock,8);
    else if ( ASSETCHAINS_SYMBOL[0] != 0 )
        kaspaz_broadcast(pblock,4);*/
    if ( KOMODO_NSPV_FULLNODE )
    {
        if ( ASSETCHAINS_SAPLING <= 0 && pindexNew->nTime > KOMODO_SAPLING_ACTIVATION - 24*3600 )
            kaspaz_activate_sapling(pindexNew);
        if ( ASSETCHAINS_CC != 0 && KOMODO_SNAPSHOT_INTERVAL != 0 && (pindexNew->nHeight % KOMODO_SNAPSHOT_INTERVAL) == 0 && pindexNew->nHeight >= KOMODO_SNAPSHOT_INTERVAL )
        {
            ...
            if ( !kaspaz_dailysnapshot(pindexNew->nHeight) )
            {
...
static bool ActivateBestChainStep(bool fSkipdpow, CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if ( KOMODO_REWIND != 0 )
    {
        ...
        LogPrintf("reached rewind.%d, best to do: ./kaspaz-cli -ac_name=%s stop\n",KOMODO_REWIND,chainName.symbol().c_str());
#ifdef WIN32
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    */
    ...
    {
        if ( it->second != 0 ) // vNodes.size() >= KOMODO_LIMITED_NETWORKSIZE, change behavior to allow kaspaz_ensure to work
        {
...
bool CheckBlockHeader(int32_t *futureblockp,int32_t height,CBlockIndex *pindex, const CBlockHeader& blockhdr, CValidationState& state, bool fCheckPOW)
{
    ...
    *futureblockp = 0;
    ...
    /*kaspaz_index2pubkey33(pubkey33,pindex,height);
                ...
                bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( fCheckPOW )
    {
        ...
        if ( ASSETCHAINS_STAKED == 0 && kaspaz_checkPOW(0,1,(CBlock *)&block,height) < 0 ) // checks Equihash
    ...
    {
        int32_t notaryid;
        int32_t special = kaspaz_chosennotary(&notaryid,height,pubkey33,tiptime);
        if (notaryid > 0 || ( notaryid == 0 && height > nS5HardforkHeight ) ) {
            ...
            if (!kaspaz_checkopret(pblockcopy, merkleroot)) {
                LogPrintf("failed or missing merkleroot expected.%s != merkleroot.%s\n", kaspaz_makeopret(pblockcopy, false).ToString().c_str(), merkleroot.ToString().c_str());
    ...
    if ( ASSETCHAINS_CC != 0 ) // CC contracts might refer to transactions in the current block, from a CC spend within the same block and out of order
    {
        ...
        while ( 1 )
        {
            ...
            for (i=0; i<block.vtx.size(); i++)
            {
                ...
                if ( tx.IsCoinBase() || !tx.vjoinsplit.empty() || !tx.vShieldedSpend.empty() || (i == block.vtx.size()-1 && kaspaz_isPoS((CBlock *)&block,height,0) != 0) )
                    continue;
                Tx = tx;
    ...
    if ( fCheckPOW && kaspaz_check_deposit(height,block) < 0 )
    {
        //static uint32_t counter;
        //if ( counter++ < 100 && ASSETCHAINS_STAKED == 0 )
        //    LogPrintf("check deposit rejection\n");
        LogPrintf("CheckBlockHeader kaspaz_check_deposit error");
        return(false);
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (fCheckpointsEnabled)
    {
        ...
        if ( nHeight != 0 )
        {
            ...
            if ( !kaspaz_checkpoint(&notarized_height,nHeight,hash) )
            {
                CBlockIndex *heightblock = chainActive[nHeight];
                if ( heightblock != 0 && heightblock->GetBlockHash() == hash )
                    return true;
...
bool ContextualCheckBlock(int32_t slowflag,const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    const int32_t txheight = nHeight == 0 ? kaspaz_block2height((CBlock *)&block) : nHeight;
    ...
    for (uint32_t i = 0; i < block.vtx.size(); i++) {
        ...
        // Interest validation
        if (!kaspaz_validate_interest(tx, txheight, cmptime))
        {
            LogPrintf("validate intrest failed for txnum.%i tx.%s\n", i, tx.ToString());
            return state.DoS(0, error("%s: kaspaz_validate_interest failed", __func__), REJECT_INVALID, "kaspaz-interest-invalid");
        }
...
bool AcceptBlockHeader(int32_t *futureblockp,const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (miSelf != mapBlockIndex.end())
    {
        ...
        /*if ( pindex != 0 && hash == kaspaz_requestedhash )
        {
            LogPrintf("AddToBlockIndex A kaspaz_requestedhash %s\n",kaspaz_requestedhash.ToString().c_str());
            memset(&kaspaz_requestedhash,0,sizeof(kaspaz_requestedhash));
            kaspaz_requestedcount = 0;
        }*/
    ...
    /*if ( pindex != 0 && hash == kaspaz_requestedhash )
    {
        LogPrintf("AddToBlockIndex kaspaz_requestedhash %s\n",kaspaz_requestedhash.ToString().c_str());
        memset(&kaspaz_requestedhash,0,sizeof(kaspaz_requestedhash));
        kaspaz_requestedcount = 0;
    }*/
    return true;
...
bool ProcessNewBlock(bool from_miner,int32_t height,CValidationState &state, CNode* pfrom, CBlock* pblock, bool fForceProcessing, CDiskBlockPos *dbp)
{
    ...
    hash = pblock->GetHash();
    {
        LOCK(cs_main);
        if ( chainActive.Tip() != 0 )
            kaspaz_currentheight_set(chainActive.Tip()->nHeight);
        checked = CheckBlock(&futureblock,height!=0?height:kaspaz_block2height(pblock),0,*pblock, state, verifier,0);
        bool fRequested = MarkBlockAsReceived(hash);
        fRequested |= fForceProcessing;
        if ( checked != 0 && kaspaz_checkPOW(0,0,pblock,height) < 0 ) //from_miner && ASSETCHAINS_STAKED == 0
        {
            checked = 0;
...
//void kaspaz_pindex_init(CBlockIndex *pindex,int32_t height);
...
bool static LoadBlockIndexDB()
{
    ...
    for (const std::pair<uint256, CBlockIndex*>& item : mapBlockIndex)
    {
        ...
        //kaspaz_pindex_init(pindex,(int32_t)pindex->nHeight);
    }
    ...
    if ( chainName.isKMD() ) {
        ...
        int32_t longestchain = kaspaz_longestchain();
        // TODO: kaspaz_longestchain does not have the data it needs at the time LoadBlockIndexDB
    ...
    if ( (pindex= chainActive.Tip()) != 0 )
    {
        if ( ASSETCHAINS_SAPLING <= 0 )
        {
            ...
            kaspaz_activate_sapling(pindex);
        }
    }
    return true;
}
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    ...
                    if (state.IsError())
                        break;
                } else if (hash != chainparams.GetConsensus().hashGenesisBlock && kaspaz_blockheight(hash) % 1000 == 0) {
                    LogPrintf("Block Import: already had block %s at height %d\n", hash.ToString(), kaspaz_blockheight(hash));
                }
...
#include "kaspaz_nSPV_defs.h"
#include "kaspaz_nSPV.h"            // shared defines, structs, serdes, purge functions
#include "kaspaz_nSPV_fullnode.h"   // nSPV fullnode handling of the getnSPV request messages
#include "kaspaz_nSPV_superlite.h"  // nSPV superlite client, issuing requests and handling nSPV responses
#include "kaspaz_nSPV_wallet.h"     // nSPV_send and support functions, really all the rest is to support this

void kaspaz_netevent(std::vector<uint8_t> payload);
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        /*CBlockIndex *pindex;
        if ( kaspaz_requestedhash != zero && kaspaz_requestedcount < 16 && (pindex= kaspaz_getblockindex(kaspaz_requestedhash)) != 0 )
        {
            LogPrint("net","kaspaz_requestedhash.%d request %s to nodeid.%d\n",kaspaz_requestedcount,kaspaz_requestedhash.ToString().c_str(),pto->GetId());
            LogPrintf("kaspaz_requestedhash.%d request %s to nodeid.%d\n",kaspaz_requestedcount,kaspaz_requestedhash.ToString().c_str(),pto->GetId());
            vGetData.push_back(CInv(MSG_BLOCK, kaspaz_requestedhash));
            MarkBlockAsInFlight(pto->GetId(), kaspaz_requestedhash, consensusParams, pindex);
            kaspaz_requestedcount++;
            if ( kaspaz_requestedcount > 16 )
            {
                memset(&kaspaz_requestedhash,0,sizeof(kaspaz_requestedhash));
                kaspaz_requestedcount = 0;
            }
