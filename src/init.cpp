...
//extern bool kaspaz_dailysnapshot(int32_t height);  //todo remove
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "kaspaz.conf"));
...
#ifndef _WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "kaspazd.pid"));
#endif
...
void CleanupBlockRevFiles()
{
    ...
    path kaspazstate = GetDataDir() / KOMODO_STATE_FILENAME;
    remove(kaspazstate);
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                {
                    if ( !kaspaz_dailysnapshot(chainActive.Height()) )
                    {
