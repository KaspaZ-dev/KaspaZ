...
#include "kaspaz_utils.h" // portable_mutex_lock
#include "kaspaz_notary.h" // kaspaz_npptr

struct kaspaz_ccdata *CC_data;
// int32_t CC_firstheight;
pthread_mutex_t KOMODO_CC_mutex; 

uint256 kaspaz_calcMoM(int32_t height,int32_t MoMdepth)
{
...
struct kaspaz_ccdata_entry *kaspaz_allMoMs(int32_t *nump,uint256 *MoMoMp,int32_t kmdstarti,int32_t kmdendi)
{
    struct kaspaz_ccdata_entry *allMoMs=0; struct kaspaz_ccdata *ccdata,*tmpptr; int32_t i,num,max;
    ...
    DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
    {
        if ( ccdata->MoMdata.height <= kmdendi && ccdata->MoMdata.height >= kmdstarti )
        {
            ...
            {
                max += 100;
                allMoMs = (struct kaspaz_ccdata_entry *)realloc(allMoMs,max * sizeof(*allMoMs));
            }
...
int32_t kaspaz_addpair(struct kaspaz_ccdataMoMoM *mdata,int32_t notarized_height,int32_t offset,int32_t maxpairs)
{
    if ( maxpairs >= 0) {
        if ( mdata->numpairs >= maxpairs )
        {
            maxpairs += 100;
            mdata->pairs = (struct kaspaz_ccdatapair *)realloc(mdata->pairs,sizeof(*mdata->pairs)*maxpairs);
            //LogPrintf("pairs reallocated to %p num.%d\n",mdata->pairs,mdata->numpairs);
        }
    } else {
        LogPrintf("kaspaz_addpair.maxpairs %d must be >= 0\n",(int32_t)maxpairs);
        return(-1);
    }
...
int32_t kaspaz_MoMoMdata(char *hexstr,int32_t hexsize,struct kaspaz_ccdataMoMoM *mdata,char *symbol,int32_t kmdheight,int32_t notarized_height)
{
    uint8_t hexdata[8192]; struct kaspaz_ccdata *ccdata,*tmpptr; int32_t len,maxpairs,i,retval=-1,depth,starti,endi,CCid=0; struct kaspaz_ccdata_entry *allMoMs;
    ...
    {
        if ( (allMoMs= kaspaz_allMoMs(&depth,&mdata->MoMoM,starti,endi)) != 0 )
        {
            mdata->MoMoMdepth = depth;
            for (i=0; i<depth; i++)
            {
                if ( strcmp(symbol,allMoMs[i].symbol) == 0 )
                    maxpairs = kaspaz_addpair(mdata,allMoMs[i].notarized_height,i,maxpairs);
            }
            if ( mdata->numpairs > 0 )
            {
...
void kaspaz_purge_ccdata(int32_t height)
{
    struct kaspaz_ccdata *ccdata,*tmpptr;
    if ( chainName.isKMD() )
    {
        portable_mutex_lock(&KOMODO_CC_mutex);
        DL_FOREACH_SAFE(CC_data,ccdata,tmpptr)
