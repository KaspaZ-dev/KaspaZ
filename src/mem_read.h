...
#include "kaspaz_structs.h" // for parse_error
...
std::size_t mem_read(T& dest, uint8_t *filedata, long &fpos, long datalen)
{
    ...
    throw kaspaz::parse_error("Invalid size: " + std::to_string(sizeof(T)) );
}
...
std::size_t mem_read(T(&dest)[N], uint8_t *filedata, long& fpos, long datalen)
{
    ...
    throw kaspaz::parse_error("Invalid size: "  + std::to_string( sz ) );
}
